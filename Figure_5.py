...
for species_code in species_codes:
    if not os.path.exists('Figure_5/ensemble_vn_UMB_Gil_v2_{}.csv'\
                          .format(species_code)):
...
for row in axs:
    for col in row:
        if i < len(species_codes):
            ...
            i += 1
            df = pd.read_csv('Figure_5/ensemble_vn_UMB_Gil_v2_{}.csv'\
                             .format(species_code))
            ...
            col.plot(date, df['observed'], color='black')
            col.plot(date, df['qt=0.5'], color=colors[species_code[:3]])
            col.fill_between(date, df['qt=0.05'], df['qt=0.95'],\
                             color='b', alpha=0.2)
            ...
            i += 1
            date = pd.to_datetime(df.date).apply(lambda x: x.strftime('%m/%d'))
            col.plot(date, df['observed'], lw=0)
